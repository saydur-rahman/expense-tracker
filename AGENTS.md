...
6. **Deactivation must be checked at every token-issuing path** (authorize, refresh, token exchange).
   ...
   **Logout must revoke tokens, not just clear the cookie**, or refresh tokens outlive the session. And the SPA's `post_logout_redirect_uri` must stay a **public** route (`/signed-out`) — pointing it at a protected one restarts sign-in immediately, which an external provider answers silently and makes logout look broken. Fix that landing page, not the provider — that is what makes logout honest, and it is a separate concern from which Google account you sign in as. The challenge **does** send `prompt=select_account`, so Google always asks which account to use (added 2026-09-02, on the owner's request): without it a single signed-in account is picked silently, and there is no way to sign in as anyone else or to retry as someone different after a failure.
...
- **Frontend server state** goes through TanStack Query; invalidate query keys after mutations. **Every query for a feature sits under that feature's own prefix** — `['loans']`, `['loans', id]`, `['loans', id, 'transactions', …]`, `['loans', 'portfolio', periodId]` — so invalidating `['loans']` invalidates all of it, including anything added later. A flat key like `['loan-portfolio']` is invisible to that and goes stale; it did.
- **`auth/jwt.ts` decodes tokens for display only** — never a security decision. The API validates every token.
- **Colour is semantic**: `brand-*` (blue) for actions and selection, `positive-*` (green) for money still held, `negative-*` (red) for trouble and destructive actions. Those three carry everything. Two more exist for exactly one job — the rungs above "your income covers your budget" on the dashboard's overview strip, where green is already taken: `surplus-*` (yellow, income 20% clear) and `gold-*` (income 50% clear). They are used as **bar fills and as tinted pills, never as ink** — yellow cannot reach 4.5:1 as text on a light card — and each rung's pill also carries a glyph so the two are told apart without relying on hue. Don't reach for either outside that ladder, and don't delete them as stray warning colours: `lib/budgetHealth.ts` owns the thresholds. **Surfaces use the semantic roles** `page`/`card`/`raised`/`input`/`track`/`line`/`line-soft`/`ink`/`ink-soft`/`ink-muted`, which resolve per theme on their own — write `bg-card`, never `bg-white dark:bg-gray-900`, and never reach for raw `gray-*`/`indigo-*`. Reuse `components/Button.tsx`, the class constants in `components/ui.ts`, and the chart primitives in `components/charts/` (`TwoSliceDonut`, `Bar`, `ProgressBar`, `PeriodBars`, `LegendRow`, and the validated marks in `colors.ts`) rather than re-typing them. **There is no charting library and there should not be one** — every visual here is inline SVG or a div with a percentage width. **If you change a chart colour, re-run the data-viz validator against both card surfaces.**
...
- **Never use `DateTime.UtcNow` or `toISOString()` as a *date*.** The server runs on UTC and has no idea where the user is; in New Zealand UTC is yesterday for most of the working day. Server-side, ask `IUserClock.Today` — it reads the caller's IANA zone from the `X-Time-Zone` header that `api/client.ts` puts on every request, falling back to UTC. Client-side, use `todayLocal()` from `lib/dates`. `CreatedAtUtc`-style timestamps stay `DateTime.UtcNow`; it is only *dates* that must be the user's.
- **The shared cycle is the date scope everywhere except loans and investments.** Dashboard, Budgets, Expenses and Income all agree on the month or week being shown, deliberately. `components/DateRangePicker` is the one exception: a loan spans years and "which cycle was that payment in" is the wrong question. Don't spread it to the four screens that share the cycle.
