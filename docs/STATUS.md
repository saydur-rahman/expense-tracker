...
- **Loans** gained a `PeriodPicker` and a totals card: still owed, borrowed, repaid, a progress bar, the percentage paid off, how many are settled, and **paid in this cycle** — the only line the picker changes, since a balance is a balance whichever month you are looking at
- **Investments & lending** gained the same per kind, so investments and lendings are added up separately and read in their own words (*still out / put in / come back* against *still owed / lent / paid back*), each with what moved in the cycle
- **`outstanding` is summed per entry, not as `borrowed − repaid`.** One overpaid loan must not quietly cancel out what is still owed on another — the aggregate would look right and be wrong
- Two new endpoints, `GET /api/loans/portfolio` and `GET /api/investments/portfolio`. The loan one reuses the existing single-pass `RepaidByLoanAsync`, now taking an optional window so the cycle figure is the same query rather than a second shape of it
- The investment portfolio always returns **both** groups in order, whether or not you use one, so the screen has one shape to render; the page hides an empty group rather than the API omitting it
- **Found while looking at it: the new cards went stale.** They were keyed `['loan-portfolio']` and `['investment-portfolio']`, which nothing invalidated — creating a lend left the totals card showing the old state. Every loan and investment query now sits under its feature's own prefix (`['loans', id, 'transactions', …]`, `['investments', 'portfolio', periodId]`), so the `['loans']` / `['investments']` invalidations that already existed cover all of them, and anything added later too. Recorded as a convention in `AGENTS.md`
- Seen in a browser: a 12,000 loan with an 800 payment read "still owed 11,200 · 6.67% paid off · Paid in Sep 2026 NZ$800.00", and stepping to **Aug 2026** left the balance untouched while the last line went to NZ$0.00 — which is the whole point of the picker being there. A 600 lend read "LENT OUT (1) · still owed 600 · 0% back · In Sep 2026 NZ$600.00 out, NZ$0.00 back"
...
- Verified against live services in a browser, both features end to end:
  - **Loans** — the More dropdown; creating a loan with a linked head; the ring wholly red at 12,000; a 500 payment dropping it to 11,500 with the per-cycle column appearing; **deleting that payment returning it to 12,000**, the case a stored balance gets wrong; the 409 refusing a head another loan already holds, naming that loan
  - **Several heads on one loan** — a second head added by editing, 300 on one and 200 on the other, both counted (500 repaid, 4.17%) and both listed with their head names
  - **The date range** — a window after both payments reads "0 payments · NZ$0.00 · nothing in that range"; widening it to include them reads "2 payments · NZ$500.00"; Clear returns to the latest 20
  - **Investments** — the two head pickers correctly offer only spending heads and only income heads; 1,000 in and 400 back gave a 40% green ring reading "still out 600", both per-cycle charts, and a merged list showing −1,000 *put in* and +400 *came back*; the invested-against-income split read 1,000 against 400 income, "over your income by 600"
  - **Removing an investment left both ledger rows in place** — the income still counted afterwards, exactly as the confirmation promises
- **The phone More sheet is now confirmed.** The extension cannot resize this window (it reports success and does nothing), so the app was mounted in a 390px iframe, which has its own viewport and so resolves the `md:` queries for real: the bottom bar shows Dashboard / Expenses / Income / More with More active, and tapping it slides the sheet up over a scrim with Loans highlighted and Log out below the divider. **This proves the portal works** — without it the sheet would have been trapped inside the 48px bar
- **Dark mode was checked arithmetically rather than rendered**, since the OS theme can't be switched from here. All ten new pairings clear their thresholds in both themes — the three chart marks on `card` (3.15–5.12:1 against a 3:1 bar) and the slate Left bar on its own `track` (3.79 light, 5.00 dark). Still worth an eye on a dark-themed browser, but the risk is low: everything else reuses the already-validated tokens
