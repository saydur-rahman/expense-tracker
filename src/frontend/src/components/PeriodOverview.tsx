...
function Bar({
  ...
  fill: string
  /** Where the budget sits on the same scale, as a percentage. Null when nothing is budgeted. */
  mark: number | null
  note?: ReactNode
}) {
  return (
    <div>
      <div className="relative h-2.5 overflow-hidden rounded-full bg-track">
        ...
        />
        {mark !== null && (
          // Drawn over the fill, so a bar that beats the budget is seen crossing it.
          <span
            aria-hidden="true"
